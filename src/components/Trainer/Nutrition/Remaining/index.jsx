...
import NutritionCycle from '../NutritionCycle'
...
const Remaining = ({
    ...
    const fetchActiveCycle = async () => {
        ...
        try {
            ...
            // )
            const response = await axios.get(
                `/diet/cycle/active?clientId=${clientId}`,
            )
            setCycle(response?.data?.activeCycle)
    ...
    const fetchRemainingCardsData = async () => {
        ...
        try {
            ...
            // )
            const response = await axios.get(
                `/dietPlan/remaining?clientId=${clientId}`,
            )
            console.log('--remaining sessions, check here ', response)
            // let updatedSessions = response?.data?.sessions?.sort(
            //     (w1, w2) => new Date(w1.date) - new Date(w2.date),
            // )
            let updatedSessions = response?.data?.sort(
                (w1, w2) => new Date(w1.date) - new Date(w2.date),
            )
    ...
    const minDateFunction = async () => {
        ...
        // )
        const tempMinDate = new Date()
        console.log('tempMinDate', tempMinDate)
        const latestCompletedSessionDate = new Date(
            new Date(latestCompletedSession?.date),
    ...
    const updateSessions = (data) => {
        const newSession = {
            ...
            foodItems: data?.routineRef?.foodItems ?? [],
            date: data?.date,
            userRef: clientId,
            editing: true,
    ...
    const addSessionHelper = () => {
        ...
        if (sessions.length > 0) {
            ...
            console.log('sessoa', lastSessionDate)
        }
        ...
        // Case where cycle exists
            ...
            if (lastRoutineIndex != null) {
                ...
                if (daysToAdd <= 0) {
                    ...
                    daysToAdd += Number(cycle?.routineRefs.length)
                }
            }
    ...
    const addAllSessions = async () => {
        ...
        let flag = 1
        while (flag) {
            console.log('cycle ', cycle, sessions)
            const sessionData = addSessionHelper()
    ...
    const saveCard = async (date, session) => {
        try {
            ...
            newSession.date = dateString.toISOString()
            // const response={"data": newSession}
            console.log('save card funtction ', newSession)
                ...
                <div className="d-flex flex-direction-row">
                    {cycle && (
                        <Button
                            ...
                            type="button"
                            classNames="add-session-btn-nutrition"
                            text={'+ Add'}
                            onClick={addSession}
                        />
                    )}
                    {cycle && (
                        <Button
                            disabled={activeCycleError}
                            type="button"
                            classNames="add-session-btn-nutrition add-cycle-session-btn"
                            text={'+ Cycle'}
